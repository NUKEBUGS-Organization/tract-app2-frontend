...
export function mapApiDeal(row: Record<string, unknown>): MarketplaceDeal {
  ...
  return {
    id,
    contractId: row.contractId != null ? refIdOnly(row.contractId) : undefined,
    ...
    wholesaler,
    wholesalerName:
      typeof row.wholesalerId === 'object' && row.wholesalerId !== null
        ? ((row.wholesalerId as { fullName?: string }).fullName ?? undefined)
        : undefined,
