    ...
    return (
      <DashboardLayout sidebar={<AdminSidebar />} className="bg-tract-alabaster font-inter text-gray-800">
        <main className="flex min-h-screen flex-col items-center justify-center gap-4 px-4">
          <p className="font-inter text-gray-500">Failed to load flagged messages.</p>
      ...
      <main className="min-h-screen px-4 py-8 md:p-10">
        <header className="sticky top-0 z-40 -mx-4 mb-6 flex w-full items-center justify-between border-b border-[#4d4635] bg-[#111417] px-4 py-3 md:-mx-10 md:px-8">
          ...
          <div className="flex flex-wrap items-center gap-4 md:gap-6">
            <button type="button" onClick={() => void refetch()} className="font-inter text-xs text-gray-400 hover:text-tract-gold">
              Refresh
            </button>
        ...
        <div className="space-y-6">
          <div className="mx-auto max-w-[1440px]">
            ...
            <p className="mt-2 max-w-3xl font-inter text-base text-gray-500">Flagged in-app messages from the database (page {page} of {data?.pages ?? 1}).</p>
          </div>
          ...
          <div className="mx-auto flex max-w-[1440px] flex-col gap-6 lg:flex-row">
            <div className="min-w-0 flex-1 rounded-xl border border-black/10 bg-white p-6">
              ...
              {rows.length === 0 ? (
                <p className="py-8 text-center font-inter text-sm text-gray-500">No flagged messages.</p>
              ) : (
                        ...
                        return (
                          <tr
                            ...
                            className={cn(
                              ...
                              selected?.id === row.id && 'bg-tract-alabaster/80',
                            )}
            ...
            <aside className="w-full shrink-0 lg:w-[400px]">
              {selected ? (
                <div className="sticky top-24 flex flex-col rounded-xl border border-black/10 bg-white p-6 shadow-lg">
                  <h4 className="mb-4 font-inter text-xs font-bold uppercase tracking-wide text-gray-500">Message context — {selected.context.dealLabel}</h4>
                  <div className="mb-6 flex max-h-[min(480px,50vh)] flex-col gap-4 overflow-y-auto pr-1">
                    {selected.context.messages.map((m) => (
                      <div key={m.id} className={cn('flex flex-col gap-1', m.align === 'right' ? 'items-end' : 'items-start')}>
                        <span className={cn('font-inter text-[10px] font-bold uppercase', m.flagged ? 'text-[#C0392B]' : 'text-gray-500', m.align === 'right' && 'text-right')}>
                          {m.meta}
                        </span>
                        <div
                          className={cn(
                ...
                <p className="rounded-xl border border-black/10 bg-white p-6 text-center font-inter text-sm text-gray-500">No message selected.</p>
              )}
            </aside>
          </div>
        </div>
