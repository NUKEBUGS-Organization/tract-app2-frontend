    ...
    return (
      <DashboardLayout sidebar={<AdminSidebar />} className="bg-theme-bg font-inter text-theme-text">
        <main className="flex min-h-screen flex-col items-center justify-center gap-4">
          ...
          <p className="font-inter text-theme-muted">Failed to load admin dashboard.</p>
          <button
            type="button"
  ...
  const STAT_CARDS = [
    {
      ...
      bg: stats?.pendingReview ? 'bg-tract-red-light' : 'bg-theme-topbar',
    ...
    },
    {
      ...
      bg: stats?.flaggedPenalties ? 'bg-tract-red-light' : 'bg-theme-topbar',
      border: 'border-tract-red/10',
      valueClass: 'text-tract-red',
      icon: ShieldAlert,
    },
    ...
    <DashboardLayout sidebar={<AdminSidebar />} className="bg-theme-bg font-inter text-theme-text">
      <main className="min-h-screen overflow-x-hidden px-4 py-8 md:p-10">
        <header className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="font-playfair text-[28px] font-bold text-theme-text">Admin Control Center</h2>
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={() => void refetch()}
              className="flex items-center gap-2 font-inter text-[12px] font-bold uppercase tracking-wider text-theme-muted hover:text-theme-text transition-colors"
            >
              <RefreshCw className="h-4 w-4" />
              Refresh
            </button>
            <div className="text-right">
              <p className="font-inter text-base font-bold text-theme-text">{displayName}</p>
              <p className="font-inter text-[13px] font-bold uppercase tracking-wider text-theme-muted">Administrator</p>
            </div>
          </div>
        </header>
          ...
          <div className="space-y-6 lg:col-span-2">
            <section className="border border-theme-border/20 bg-theme-card p-8 shadow-sm">
              ...
              {listings.length === 0 ? (
                <div className="flex flex-col items-center py-8 text-center gap-3">
                  ...
                  <p className="font-inter text-[14px] text-theme-muted">No listings pending review.</p>
                </div>
              ) : (
                      ...
                      <tr className="border-b border-theme-border">
                        {['Property', 'Wholesaler', 'Submitted', 'Flag', 'Action'].map((h) => (
                          <th
                            key={h}
                            className={cn(
                              'py-4 font-inter text-xs font-bold uppercase text-theme-muted',
                              h === 'Action' && 'text-right',
                            )}
                          >
                            {h}
                    ...
                    <tbody className="divide-y divide-theme-border">
                      {listings.map((listing) => (
                        <tr key={listing.id} className="transition-colors hover:bg-theme-surface-2">
                          <td className="py-5 font-inter text-base font-bold text-theme-text">
                            {listing.propertyAddress}
                            {listing.city ? `, ${listing.city}` : ''}
                          </td>
                          <td className="py-5 font-inter text-sm text-theme-muted">{listing.wholesalerName}</td>
                          <td className="py-5 font-inter text-sm text-theme-muted">{listing.submittedAt}</td>
                          <td className="py-5">
                            <span
                              className={cn(
            ...
            <section className="border border-theme-border/20 bg-theme-card p-8 shadow-sm">
              ...
              {penalties.length === 0 ? (
                <div className="flex flex-col items-center py-8 text-center gap-3">
                  <CheckCircle2 className="h-10 w-10 text-tract-green" strokeWidth={1} />
                  <p className="font-inter text-[14px] text-theme-muted">No penalties recorded.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {penalties.map((p) => (
                    ...
                    >
                      <div>
                        <p className="font-inter text-base font-bold text-theme-text">
                          {p.violationLabel} <span className="font-normal text-theme-muted">— {p.userName}</span>
                        </p>
                      ...
                      <span
                        className={cn(
                          ...
                          p.resolved ? 'bg-theme-surface-2 text-theme-muted' : 'bg-tract-red-light text-tract-red',
                        )}
                      >
          ...
          <div className="lg:col-span-1">
            <section className="flex h-full min-h-[420px] flex-col border border-theme-border/20 bg-theme-card p-8 shadow-sm">
              ...
              <div className="mb-10">
                ...
                <p className="mt-1 font-inter text-sm italic text-theme-muted">Platform fees collected</p>
              </div>
              <div className="mt-auto border-t border-theme-border pt-8">
                <div className="mb-3 flex justify-between font-inter text-sm">
                  <span className="text-theme-muted">Live listings</span>
                  <span className="font-bold text-tract-gold">{stats?.liveListings ?? 0}</span>
                </div>
                <div className="mb-3 flex justify-between font-inter text-sm">
                  <span className="text-theme-muted">Active deals</span>
                  <span className="font-bold text-theme-text">{stats?.activeDeals ?? 0}</span>
                </div>
              </div>
            </section>
          </div>
