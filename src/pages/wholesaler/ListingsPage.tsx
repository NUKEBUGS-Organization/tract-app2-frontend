    ...
    <div className="flex min-h-screen bg-tract-alabaster font-inter text-tract-obsidian">
      ...
      <main className="ml-[240px] min-h-screen flex-1">
        ...
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-gray-100 bg-white px-6 md:px-12">
          <h2 className="font-playfair text-[22px] font-bold text-tract-obsidian">My Listings</h2>
          <Link
        ...
        <div className="mx-auto max-w-[1440px] p-6 md:p-12">
          ...
          {!isLoading && !isError && listings.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="mb-2 font-playfair text-[24px] font-bold text-tract-obsidian">No listings yet</h3>
              <p className="mb-8 max-w-xs font-inter text-gray-500">
                  ...
                  {listings.map((listing) => {
                    ...
                    return (
                      <tr key={listing.id} className="transition-colors hover:bg-gray-50">
                        ...
                        <td className="px-6 py-5 font-inter text-[13px] text-gray-500">{dealLabel}</td>
                        <td className="px-6 py-5 font-inter text-[13px] font-semibold text-tract-obsidian">
                        ...
                        <td className="px-6 py-5 font-inter text-[13px] text-gray-500">{listing.bidCount ?? 0} / 10</td>
                        <td className="px-6 py-5">
                          <span
                            className={cn(
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex items-center justify-end gap-3">
                            ...
                            {listing.status === 'under_contract' && (
                              <Link
                                to={`/wholesaler/listings/${listing.id}`}
                                className="font-inter text-[12px] font-bold uppercase tracking-wider text-tract-obsidian hover:underline"
                              >
                                View Deal
                              </Link>
                            )}
