...
export default function BuyerListingDetailPage() {
  ...
  if (isError || !listing) {
    return (
      <DashboardLayout sidebar={<Sidebar />}>
        <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-4 text-center font-poppins text-app1-text-muted">
  ...
  return (
    <DashboardLayout sidebar={<Sidebar />}>
      <div className="mx-auto w-full max-w-[1440px] flex-1 p-6 md:p-10">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(320px,380px)] lg:items-start lg:gap-8">
          ...
          <div className="min-w-0 w-full">
            <aside className="sticky top-6 flex w-full min-w-0 flex-col gap-6 overflow-hidden lg:top-8">
              <div className="w-full min-w-0 overflow-hidden rounded-app1-card border border-app1-border-light bg-app1-bg-card p-6 shadow-app1-card">
                ...
                {bidAllowance.data ? (
                  <p className="mb-4 rounded-lg bg-app1-bg-soft px-3 py-2 font-poppins text-xs text-app1-text-muted">
                    {bidAllowance.data.remaining} of {bidAllowance.data.freeLimit} free bid submissions remaining
                  </p>
                ) : null}
                ...
                {!listing.bidsOpen ? (
                  <p className="font-poppins text-sm text-app1-text-muted">Bidding closed for this listing.</p>
                ) : (
                  <form onSubmit={handleSubmit(onBidSubmit)}>
                    ...
                    <button
                      type="submit"
                      disabled={
                        !canPlaceBid || placeBid.isPending || !listing.bidsOpen || isKycPending || isPofPending
                      }
