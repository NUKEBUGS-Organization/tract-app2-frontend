...
import { useSubscription, useSubscriptionAction } from '@/hooks/useSubscription'
...
export default function SubscriptionGate({ children }: { children: ReactNode }) {
  ...
  if (status.data?.active) return <>{children}</>
...
export function SubscriptionPanel() {
  ...
  return <section className="rounded-xl border border-app1-border-light bg-app1-bg-card p-6 text-app1-text-main space-y-4">
    ...
    {status.data?.required === false ? <p>Your role does not require a subscription.</p> : <>
      ...
      {status.data?.active ? <p role="status">Paid access through {new Date(status.data.paidUntil!).toLocaleDateString()}{status.data.status === 'CANCELLED' ? '. Renewal cancelled.' : '.'}</p> : <>
        <label className="flex items-start gap-3 text-sm"><input type="checkbox" checked={accepted} onChange={(e) => setAccepted(e.target.checked)} className="mt-1" />
        ...
        <button disabled={!accepted || subscribe.isPending || !status.data} onClick={() => subscribe.mutate()} className="rounded-lg bg-app1-secondary px-5 py-3 text-app1-primary-dark disabled:opacity-50">{subscribe.isPending ? 'Opening PayPal…' : 'Subscribe with PayPal'}</button>
      </>}
